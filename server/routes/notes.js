...
router.get('/:id', auth, async (req, res) => {
  try {
    ...
    const isCollaborator = note.collaborators.some(c => c._id.toString() === req.user.userId);
...
router.delete('/:id', auth, async (req, res) => {
  try {
    const note = await Note.findById(req.params.id);
    if (!note) return res.status(404).json({ message: 'Note not found' });
    if (note.owner.toString() !== req.user.userId) return res.status(403).json({ message: 'Only owners can delete notes' });
...
router.post('/:id/share', auth, async (req, res) => {
  try {
    ...
    const userToShare = await User.findOne({ $or: [{ email: identity }, { username: identity }] });
    if (!userToShare) return res.status(404).json({ message: 'User not found' });

    const note = await Note.findById(req.params.id);
    if (note.owner.toString() !== req.user.userId) return res.status(403).json({ message: 'Only owners can share' });

    if (!note.collaborators.includes(userToShare._id)) {
      note.collaborators.push(userToShare._id);
      await note.save();
    }
    res.json({ message: 'Note shared successfully', collaborator: userToShare.username });
  } catch (err) {
...
router.post('/:id/comment', auth, async (req, res) => {
  try {
    ...
    const populatedNote = await Note.findById(req.params.id).populate('comments.author', 'username');
